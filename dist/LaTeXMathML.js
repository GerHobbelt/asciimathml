...
var LMroot = { input: "\\root", tag: "mroot", output: "root", ttype: BINARY };
var LMfrac = { input: "\\frac", tag: "mfrac", output: "/", ttype: BINARY };
var LMover = { input: "\\stackrel", tag: "mover", output: "stackrel", ttype: BINARY };
var LMatop = { input: "\\atop", tag: "mfrac", output: "", ttype: INFIX };
var LMchoose = { input: "\\choose", tag: "mfrac", output: "", ttype: INFIX };
var LMsub = { input: "_", tag: "msub", output: "_", ttype: INFIX };
var LMsup = { input: "^", tag: "msup", output: "^", ttype: INFIX };
var LMtext = { input: "\\mathrm", tag: "mtext", output: "text", ttype: TEXT };
var LMmbox = { input: "\\mbox", tag: "mtext", output: "mbox", ttype: TEXT };
...
function LMcreateElementMathML(t) {
  if (isIE) {
    ...
    return document.createElementNS(LMmathml, t);
  }
}
...
function LMcreateMmlNode(t, frag) {
  ...
  if (isIE) {
    ...
    var node = document.createElementNS(LMmathml, t);
  }
...
function LMparseSexpr(str) {
  ...
  case TEXT:
    ...
    newFrag.appendChild(LMcreateMmlNode(symbol.tag, document.createTextNode(st)));
  ...
  case UNARY:
    ...
    if (typeof symbol.func === "boolean" && symbol.func) {
      ...
      if (st === "^" || st === "_" || st === ",") {
        ...
        node = LMcreateMmlNode("mrow", LMcreateMmlNode(symbol.tag, document.createTextNode(symbol.output)));
        if (isIE) {
          var space = LMcreateElementMathML("mspace");
    ...
    if (symbol.input == "\\sqrt") {
      ...
      // font change or displaystyle command
        ...
        for (i = 0; i < result[0].childNodes.length; i++) {
          if (result[0].childNodes[i].nodeName == "mi" || result[0].nodeName == "mi") {
            ...
            if (result[0].nodeName == "mi") {
              result[0] = LMcreateElementMathML("mo").appendChild(document.createTextNode(newst));
            } else {
              result[0].replaceChild(LMcreateElementMathML("mo").appendChild(document.createTextNode(newst)), result[0].childNodes[i]);
            }
          }
        }
      }
  ...
  case BINARY:
    result = LMparseSexpr(str);
    if (result[0] == null) {
      return [LMcreateMmlNode("mo", document.createTextNode(symbol.input)), str, null];
    }
  ...
  case INFIX:
    str = LMremoveCharsAndBlanks(str, symbol.input.length);
    return [LMcreateMmlNode("mo", document.createTextNode(symbol.output)), str, symbol.tag];
...
function LMparseMath(str) {
  var result;
  var node = LMcreateElementMathML("mstyle");
  if (mathcolor != "") {
